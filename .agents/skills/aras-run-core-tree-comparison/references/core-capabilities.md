...
| Formal entry point | `CoreTreeComparisonCommand` | Planned command/action that binds the case snapshot and invokes the builder. It is **尚未建立**; the Skill must stop with `Blocked` until it exists. |

The formal command/action, CLI, UI, and external Aras/DB adapters are not currently present. Core-only tests are offline fixtures and do not constitute a customer execution.
