...
| `aras-compare-core-tree` | Phase 5 比較、人工核准、完成判定與獨立 A／B／C 交付核心／CLI／Skill 已建立 | 比較及分類 Core Tree；建立完成判定收據；再從已驗證證據建立新的 A／B／C delivery | 客戶來源、來源 OOTB、R38 OOTB、三份版本證據、內容比較規則版本；完成判定另需原 attempt 與人工核准收據；交付另需 completion receipt、`classification-result.json` 與三份 input tree digest | 分類、人工確認、`Incomplete` attempt、completion receipt；交付成功後於 `core-tree/deliveries` 產生 A／B／C 與 `delivery-manifest.json` | 輸入不可變；多候選不猜測；每次 attempt／completion／delivery 使用新目錄；完成判定不得改寫原 attempt；交付前重驗 evidence checksum 與 input digest；同一 comparison attempt 只允許一份 delivery | `aras-manage-upgrade-case`、`aras-run-core-tree-comparison`、`aras-build-core-tree-delivery`；`CoreTreeComparisonCommand`、`CoreTreeManualReviewApprovalCommand`、`CoreTreeComparisonFinalizationCommand`、`CoreTreeDeliveryCommand` |
