...
| `aras-prepare-ootb-hop-diff` | 後續建立 | 建立或驗證 Rule 1 OOTB 跳點差異包 | 來源／目標 OOTB `Solutions`、規則版本、工作目錄 | `SourceDiff`、`TargetDiff`、摘要、完成標記、封裝 Checksum | 原始輸入不可變；先建工作副本；AML 人工確認未解除不得完成 | `aras-manage-upgrade-case`、`aras-manage-upgrade-rules`；AML Standard；Rule 1 command/action |
| `aras-prepare-adapted-package` | 後續建立 | 使用 Rule 2 準備特定跳點正式適配 Package | 客戶 Package 基準、`TargetDiff`、客戶 `Support\Solutions`、規則版本 | 來源工作副本、原始 Solutions 備份、候選／正式適配 Package、摘要 | 備份成功前不得寫入；只處理 XML；人工確認未解除不得交付；工作目錄鎖 | `aras-manage-upgrade-case`、`aras-manage-upgrade-rules`、`aras-prepare-ootb-hop-diff`；AML Standard；Rule 2 command/action |
...
- Package／AML、Core Tree、跳點與交付：正式程式能力與對應功能 Skill 同步建立，不先建立沒有受測執行能力的空殼 Skill。
