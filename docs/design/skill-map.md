...
| `aras-compare-core-tree` | 後續建立 | 比較及分類 Core Tree | 客戶來源、來源 OOTB、R38 OOTB、三份版本證據、內容比較規則版本；`source-provenance.md` 僅為補充追溯，不取代 `version-primary.*`／`integrity.*` | A／B／C、待人工確認清單、摘要、`Incomplete`／`Completed` 標記 | 三份輸入驗證；正式比較前 preflight 必須為 `Ready`；證據不足或結構錯誤不得進入正式比較；不得自動推定版本已驗證；不合併或修改 R38；多候選不猜測；每次嘗試新輸出；重疊目錄鎖；`Completed` 僅能由正式流程在必要步驟完成後建立 | `aras-manage-upgrade-case`；Core Tree command/action |
