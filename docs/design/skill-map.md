...
| `aras-compare-core-tree` | Phase 5 比較、人工核准與完成判定核心／CLI／Skill 已建立；完整 A／B／C 交付仍由獨立受控流程處理 | 比較及分類 Core Tree，並對已核准、零錯誤的 `Incomplete` attempt 建立獨立完成判定收據 | 客戶來源、來源 OOTB、R38 OOTB、三份版本證據、內容比較規則版本；單一來源可接受 `source-provenance.md` 作最低追溯，但不足以自動視為已驗證；完成判定另需原 attempt、人工核准收據與新 completion 目錄 | A／B／C 分類結果、待人工確認清單、摘要、`Incomplete` attempt；核准後在新目錄產生 `completion-manifest.json` 並追加 `core-tree.comparison.completed` | 三份輸入驗證；來源證據不足仍可進入比較但必須標記為 `Incomplete`；不得以證據不足自動阻斷既有案件推進；不得自動推定版本已驗證；不合併或修改 R38；多候選不猜測；每次嘗試新輸出；重疊目錄鎖；完成判定不得改寫原 attempt，且不得被視為已建立完整 A／B／C 交付 | `aras-manage-upgrade-case`、`aras-run-core-tree-comparison`、`aras-build-core-tree-delivery`；`CoreTreeComparisonCommand`、`CoreTreeManualReviewApprovalCommand`、`CoreTreeComparisonFinalizationCommand` |
