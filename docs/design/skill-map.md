...
|---|---|---|---|---|---|---|
| `aras-manage-upgrade-case` | 第一階段已建立 | 建立、開啟、檢查或更新 Core Tree／Package／DB 工作流；建立新版升級路徑；查看任務、關卡、歷程、中斷或重試資格 | 案件根目錄、案件欄位、Core Tree 設定或版本化跳點、操作者、歷程 | 案件清單、各工作流任務圖、目前狀態、阻擋原因、可追溯的規劃或歷程動作 | Core Tree 僅驗證三份輸入與 evidence，不要求 Package／DB 路徑；Package／DB 路徑仍須連續；既有路徑不可改寫；歷程只追加；重試須有證據；受控動作使用安全判定與目錄鎖 | 正式核心 `Cases`、`Tasks`、`Execution`、`Safety`；其他功能 Skill 共用案件識別及歷程 |
...
| `aras-compare-core-tree` | Phase 5 目前版本至最終版本的比較、人工核准、完成判定與獨立 A／B／C 交付核心／CLI／Skill 已建立 | 比較及分類 Core Tree；建立完成判定收據；再從已驗證證據建立新的 A／B／C delivery | Core Tree 工作流的 `coreTreeComparison` 指定客戶目前版本、同版 OOTB、最終版 OOTB、三份版本證據、內容比較規則版本；不要求 Package／DB 路徑；完成判定另需原 attempt 與人工核准收據；交付另需 completion receipt、`classification-result.json` 與三份 input tree digest | 分類、人工確認、`Incomplete` attempt、completion receipt；交付成功後於 `core-tree/deliveries` 產生 A／B／C 與 `delivery-manifest.json` | 輸入不可變；不依 Package／DB 中間跳點或路徑建立；多候選不猜測；每次 attempt／completion／delivery 使用新目錄；完成判定不得改寫原 attempt；交付前重驗 evidence checksum 與 input digest；同一 comparison attempt 只允許一份 delivery | `aras-manage-upgrade-case`、`aras-run-core-tree-comparison`、`aras-build-core-tree-delivery`；`CoreTreeComparisonCommand`、`CoreTreeManualReviewApprovalCommand`、`CoreTreeComparisonFinalizationCommand`、`CoreTreeDeliveryCommand` |
...
Phase 5 comparison finalization uses `CoreTreeComparisonFinalizationCommand` through CLI `--finalize-comparison`. It accepts only an approved, zero-error `Incomplete` attempt, validates immutable review checksums and append-only history, writes a new receipt under `core-tree/completions`, and never rewrites the attempt. This receipt completes the comparison/review decision only; A／B／C delivery construction remains a separate controlled capability.

Phase 5 independent delivery uses `CoreTreeDeliveryCommand` through CLI `--build-delivery`. It requires the completion receipt, the immutable `classification-result.json`, unchanged input tree digests, and formally resolved manual decisions; it writes one new A／B／C delivery under `core-tree/deliveries` and appends `core-tree.delivery.completed`.
